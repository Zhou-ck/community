<template>
  <view>
    ...
    <canvas canvas-id="hxkxt" id="hxkxt" class="charts" @touchstart="touchstart"/>
  </view>
</template>
	...
	watch:{
		sleepWeekData: {
		    handler(newValue, oldValue) {  
		      // console.log("sleepWeekData或其内部值发生了变化",newValue);  
		      // 这里可以处理你的逻辑
			  this.getServerData();
		    },  
		    deep: true, // 启用深度监听  
    ...
    getServerData() {
      ...
      setTimeout(() => {
        ...
        let res = {
		  ...
		  res.series[0].data=this.sleepWeekData.series[4].data
        this.drawCharts('hxkxt', res);
      }, 500);
    },
    drawCharts(id,data){
