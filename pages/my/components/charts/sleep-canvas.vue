<template>
	<view class="chartBox">
		...
		<view style="width:100%;text-align: center;">{{startTime}}&nbsp;&nbsp;~&nbsp;&nbsp;{{endTime}}</view>
		<!-- 睡眠信息展示 -->
		<view class="sleep_info" v-if="Object.keys(touchValue).length != 0">
			<view class="info_box" :style="{'background-color': colors[touchValue.sleepState]}">
				...
				<text class="state_text">{{touchValue.sleepStateText}}</text>
			</view>
		</view>
		<view class="sleep_info" v-else>
			<view class="info_box" style="background: rgba(62, 209, 150, 1.0);">
				...
				<text class="state_text" style="color: #000;">睡眠</text>
			</view>
		</view>
		...
		<movable-area class="movaBox">
			<movable-view class="movaDot" direction="horizontal" @change="onChange" @touchstart="onTouchStart"
				@touchend="onTouchEnd">
...
<script>
	...
	import {formatSleepCanvasForTenDatas} from '@/utils/handleSleepReport.js'
	...
	export default {
		...
		methods: {
			...
			formatSleepData(){
				...
				this.sleepData = formatSleepCanvasForTenDatas(this.sleepReport);
				// console.log(this.sleepData);
				this.init();
			},
			...
			init() {
				...
				for (let index = 0; index < this.sleepData.length; index++) {
					...
					if (index < this.sleepData.length - 1) {
						...
						item.line = {
							x1: xValue + xWidth,
							y1: yValue[state] + (state > nextState ? yHeight - this.radius : this.radius),
							x2: xValue + xWidth,
							y2: yValue[nextState] + (state > nextState ? this.radius : yHeight - this.radius)
						};
					}
			...
			handleSleepcharts(e) {
				...
				const dotWidth = 36; // movaDot的宽度（rpx转px）
